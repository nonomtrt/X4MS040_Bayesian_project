...
These are the initialization states for the model(NOT FINISHED):
...
model <- function(Ndoses, Nplates, x, y, alpha, beta, gamma, tau, lambda, 
  ...
  for(k in 1:nchain){
    ...
    tau.shape <- tau.phi + .5*sum(lambda**2)
    tau <- rgamma(n = 1, rate = tau.rate, shape = tau.shape)
    chain[k + 1, 4] <- tau
    lambda <- structure(rnorm(n = Nplates * Ndoses, mean = 0, sd = tau), .Dim = c(Ndoses, Nplates))
    for(j in 1:Ndoses){
      for(i in 1:Nplates){
        mu[j, i] <- exp(alpha + beta * log(x[j] + 10) + gamma * x[j] + lambda[j, i])
      }
    }
    # Metropolis-Hastings for alpha
    alpha.candidate <- rnorm(1, mean = chain[k, 1], sd = prop_sd[1])
    alpha.top <- alpha^2 / alpha.sigma2 + sum(y * log(mu) - mu)
    alpha.bottom <- chain[k, 1]^2 / alpha.sigma2 + sum(y * log(mu) - mu)
    alpha.ratio <- alpha.top - alpha.bottom
    if(log(runif(1)) < alpha.ratio){
    ...
    beta.top <- beta^2 / beta.sigma2 + sum(y * log(mu) - mu)
    beta.bottom <- chain[k, 2]^2 / beta.sigma2 + sum(y * log(mu) - mu)
    beta.ratio <- beta.top - beta.bottom
    if(log(runif(1)) < beta.ratio){
    ...
    gamma.top <- gamma^2 / gamma.sigma2 + sum(y * log(mu) - mu)
    gamma.bottom <- chain[k, 3]^2 / gamma.sigma2 + sum(y * log(mu) - mu)
    gamma.ratio <- gamma.top - gamma.bottom
    if(log(runif(1)) < gamma.ratio){
...
nchain <- 5000
prop_sd <-  c(3,3,3)
chain <- model(Ndoses, Nplates, x, y, alpha, beta, gamma, tau, lambda, nchain,
               prop_sd)
```
