...
model <- function(Ndoses, Nplates, x, y, alpha, beta, gamma, tau, lambda, 
  ...
  for(k in 1:nchain){
    ...
    chain[k + 1, 4] <- tau
    lambda <- structure(rnorm(n = Nplates * Ndoses, mean = 0, sd = chain[k + 1, 4]), .Dim = c(Ndoses, Nplates))
    for(j in 1:Ndoses){
      for(i in 1:Nplates){
    ...
    alpha.top <- alpha.candidate^2 / alpha.sigma2 + sum(y * log(mu.candidate) - mu.candidate)
    ...
    beta.top <- beta.candidate^2 / beta.sigma2 + sum(y * log(mu.candidate) - mu.candidate)
    ...
    gamma.top <- gamma.candidate^2 / gamma.sigma2 + sum(y * log(mu.candidate) - mu.candidate)
